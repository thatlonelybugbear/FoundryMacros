...
const shieldItems = actor.itemTypes.equipment.filter(it=>it.data.data.armor?.type === "shield")
...
updates = updates.concat(actor.items.filter(i => i.name !== weaponMain.name && i.name !== weaponSecondary?.name).filter(i=> i.type==="weapon" || i.getRollData().item.armor?.type === "shield").map(i=> {
    return { '_id':i.id, 'data.equipped':false }}))
...
async function removeDualWieldingACbonus(actor) {
    await actor.effects.find(eff=>eff.data.label === "Dual Wielder AC boost")?.delete()
}
...
async function revertWeapons(weapons) {
    ...
    let updateparts=[];
    let weaponsTodeleteIDs = weapons.filter(i=>i.data.flags.world?.originalWeaponLoadout?.originalWeaponId).map(i=>i.id)
    ...
    if (!!weaponsTodeleteIDs.length) {
        if (weaponsTodeleteIDs.length > 1) {
            ...
            if (originalWeaponId1 === originalWeaponId2) {
                ...
                await originalWeapon.update({"data.quantity":originalWeapon.getRollData().item.quantity + 2})
                ...
                updateparts = [{'_id':originalWeaponId1,'data.quantity': originalWeapon1quantity + 1},{'_id':originalWeaponId2,'data.quantity': originalWeapon2quantity + 1}]
            ...
            updateparts = [{'_id':originalWeaponId, 'data.quantity': originalWeaponQuantity + 1}];
...
//Secondary weapon
    ...
    let mainFlag = {'originalWeaponLoadout': { 'updateWeapon':{ '_id': weaponM.id, 'name':weaponM.name,  'data.damage':damageM, 'data.range':rangeM, 'data.actionType': actionTypeM } } }; 
    let secondaryFlag;
    if (weaponS) secondaryFlag = {'originalWeaponLoadout': { 'updateWeapon': { '_id': weaponS.id, 'name':weaponS.name, 'data.damage':damageS, 'data.range':rangeS, 'data.activation': activationS } } };
    if (weaponM) {
//mwak - 2-Handed weapons or Main weapon that is not to be used as versatile or under dueling feature.
        if (propertiesM.two && !isVersatile && actionTypeM !== "rwak"|| actionTypeM !== "rwak" && !isDueling && !isVersatile || actionTypeM === "rwak" && !sharpShooter) {
            updateM = { '_id': weaponM.id, 'name': `(main) ${weaponM.name}`, 'flags.world': mainFlag, 'data.equipped': true }
        }
//rwak - 2-handed weapons like bows, (with and without sharpshooter feat)
            ...
            newRangeM.long = ""
            updateM = { '_id': weaponM.id, 'name': `(main) ${weaponM.name}`, 'flags.world': mainFlag, 'data.range': newRangeM, 'data.equipped': true }
        }
//Versatile weapons
        else if (isVersatile) {
            ...
            if(!!damageM.versatile) {
                updateM = { '_id': weaponM.id, 'name': `(vers) ${weaponM.name}`, 'flags.world': mainFlag, 'data.damage':newDamage, 'data.equipped': true }
            }
                ...
                updateM = {  '_id': weaponM.id, 'name': `(vers) ${weaponM.name}`, 'flags.world': mainFlag, 'data.equipped': true }
            }
        }
//Dueling
        else if (isDueling) {
            ...
            updateM = {  '_id': weaponM.id, 'name': `(main) ${weaponM.name}`, 'flags.world': mainFlag, 'data.damage':newDamage, 'data.equipped':true }
        }
    }   
    if (weaponS) {
//Take care of shields first
        if(weaponS.type === "equipment" ) {
            updateS = {  '_id': weaponS.id, 'data.equipped':true }
        }
        else {
            ...
            if(!twoWeaponFighter) {
                ...
                updateS = {  '_id': weaponS.id, 'name': `(off-hand) ${weaponS.name}`, 'flags.world': secondaryFlag, 'data.damage':newDamage, 'data.activation':newActivationTypeS, 'data.equipped':true }
            }
            else {
                updateS = {  '_id': weaponS.id, 'name': `(off-hand) ${weaponS.name}`, 'flags.world': secondaryFlag, 'data.activation':newActivationTypeS, 'data.equipped':true }
            }
        }
    }
    let returns 
...
async function splitWeapon(weaponId, hand) {
    ...
    if(hand!=="both") {
        ...
        let newWeapon = origWeapon.data.toObject();
        ...
        newWeapon.data.quantity = 1;
        if (origWeapon.getRollData().item.actionType === "rwak") {
            newWeapon.data.consume = {
                "type": "ammo",
        ...
        returns = [newItems[0].id, {'_id':origWeapon.id, 'data.equipped':false, 'data.quantity':origWeapon.getRollData().item.quantity - 1}]
        return returns;
    }  
    else {   //(hand==="both")
        const origWeapon = actor.items.get(weaponId);
        let newWeaponM = origWeapon.data.toObject();
        newWeaponM.data.quantity = 1;
        let newWeaponS = origWeapon.data.toObject();
        newWeaponS.data.quantity = 1;
        if (origWeapon.getRollData().item.actionType === "rwak") {
            newWeaponM.data.consume = {
                "type": "ammo",
                "target": origWeapon.id,
                "amount": 1,
            }
            newWeaponS.data.consume = {
                "type": "ammo",
                "target": origWeapon.id,
                "amount": 1,
            }
        ...
        returns = [newItems[0].id, newItems[1].id, {'_id':origWeapon.id, 'data.equipped':false, 'data.quantity':origWeapon.getRollData().item.quantity - 2}]
        return returns;
    }
}
