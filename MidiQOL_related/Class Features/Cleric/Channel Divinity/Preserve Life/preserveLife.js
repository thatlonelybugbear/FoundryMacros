const healingPool = 5*(args[0].actor.getRollData().classes?.cleric?.levels);
...
function getValidTargets(token, disposition = null, radius, maxSize = undefined) {
    const targets = MidiQOL.findNearby(disposition, token, radius, maxSize).filter(tok=>tok.actor.system.attributes.hp.value < tok.actor.system.attributes.hp.max);
    return targets.filter(tok=>!["undead","construct"].some(v=>tok.actor.system.details.type?.value.includes(v)) || !["undead","construct"].some(v=>tok.actor.system.details.race.toLocaleLowerCase().includes(v)))
}
...
function targetSelectedContent({lock, slider, id, value, lockedTargets, dialogContent, specificContent, availableTargets}) {
    ...
    if (!lock && !slider) {
        ...
        if (initialTargets.filter(tok=>!game.sliderHealing.includes(tok)).length > 0) {
    ...
    for (let target of game.sliderHealing) {
        ...
        let missingHp = max - value; //Math.min(max - value,Math.floor(max/2));
        let maxTobeHealed = Math.min(missingHp, Math.floor(max/2))
        neededHealing += maxTobeHealed
        neededHpPerTarget.push([target.id,maxTobeHealed,missingHp])
    }
    //@no slider change
    if (!slider && !lock) {
        for (let target of game.sliderHealing) {
            ...
            let ttk = neededHpPerTarget.find(key => key[0] === target.id)[2]
...
async function targetingDialog(content, initialTargets, healingPool) {
    ...
    const mainDialog = await new Promise((resolve, reject) => {
        d = new Dialog({
            title: `Preserve Life targets selection`,
            content,
            render: (html) => {
